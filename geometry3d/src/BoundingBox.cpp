...
AABB::AABB(const Point3D& min, const Point3D& max) noexcept
    : m_min(min), m_max(max) {
}
...
void AABB::reset() noexcept {
    m_min = Point3D(std::numeric_limits<double>::max(),
                    std::numeric_limits<double>::max(),
                    std::numeric_limits<double>::max());
    m_max = Point3D(std::numeric_limits<double>::lowest(),
                    std::numeric_limits<double>::lowest(),
                    std::numeric_limits<double>::lowest());
}
...
bool BoundingBox::contains(const Point3D& point) const noexcept {
    ...
    const Vec3D v(point.x() - mCenter.x(),
                  point.y() - mCenter.y(),
                  point.z() - mCenter.z());
...
AABB BoundingBox::toAABB() const noexcept {
    ...
    for (int i = 0; i < 8; ++i) {
        ...
        const Point3D worldCorner(
            mCenter.x() + corner.x() * mAxes[0].x() + corner.y() * mAxes[1].x() + corner.z() * mAxes[2].x(),
            mCenter.y() + corner.x() * mAxes[0].y() + corner.y() * mAxes[1].y() + corner.z() * mAxes[2].y(),
            mCenter.z() + corner.x() * mAxes[0].z() + corner.y() * mAxes[1].z() + corner.z() * mAxes[2].z()
        );
...
OBB::OBB() noexcept
    : mFrame(CoordinateFrame3D::worldFrame())
    , mHalfExtents(0.0, 0.0, 0.0) {
}
...
OBB::OBB(const Point3D& center, const Vec3D& xAxis, const Vec3D& yAxis, 
         const Vec3D& zAxis, const Vec3D& halfExtents)
    : mFrame(center, xAxis, yAxis, zAxis)
    , mHalfExtents(halfExtents) {
}
...
OBB OBB::fromPoints(const std::vector<Point3D>& points, const CoordinateFrame3D& frame) {
    ...
    Vec3D minLocal(std::numeric_limits<double>::max(), 
                   std::numeric_limits<double>::max(), 
                   std::numeric_limits<double>::max());
    Vec3D maxLocal(std::numeric_limits<double>::lowest(), 
                   std::numeric_limits<double>::lowest(), 
                   std::numeric_limits<double>::lowest());
...
Vec3D OBB::size() const noexcept {
    return mHalfExtents * 2.0;
}
...
std::array<Point3D, 8> OBB::getCorners() const noexcept {
    ...
    for (int i = 0; i < 8; ++i) {
        const Point3D localCorner(
            (i & 1) ? mHalfExtents.x() : -mHalfExtents.x(),
...
bool OBB::intersects(const OBB& other) const noexcept {
    ...
    const Vec3D axes[15] = {
        ...
        mFrame.xAxis().cross(other.mFrame.xAxis()),
        mFrame.xAxis().cross(other.mFrame.yAxis()),
        mFrame.xAxis().cross(other.mFrame.zAxis()),
        mFrame.yAxis().cross(other.mFrame.xAxis()),
        mFrame.yAxis().cross(other.mFrame.yAxis()),
        mFrame.yAxis().cross(other.mFrame.zAxis()),
        mFrame.zAxis().cross(other.mFrame.xAxis()),
        mFrame.zAxis().cross(other.mFrame.yAxis()),
        mFrame.zAxis().cross(other.mFrame.zAxis())
    };
    
    const Vec3D centerDiff(
    ...
    for (const auto& axis : axes) {
        ...
        const Vec3D normalizedAxis = axis.normalized();
        
        // Project both OBBs onto the axis
        const double thisProjection = projectOntoAxis(normalizedAxis);
        const double otherProjection = other.projectOntoAxis(normalizedAxis);
        
        // Distance between centers along this axis
        const double centerDistance = std::abs(centerDiff.dot(normalizedAxis));
        
        // If the distance between centers is greater than the sum of projections,
        // we found a separating axis
        if (centerDistance > thisProjection + otherProjection) {
            return false;
        }
    }
